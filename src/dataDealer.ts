...
const main = async (initialPostNumber: number) => {
  ...
  while(true){
    try{
      ...
      let POST100: number = currentPost+5
      const isPost100Exist: boolean = await checkPostExist(POST100)
      if(isPost100Exist){
        console.log(currentPost, currentPost + 5)
        ...
        for(let i = 0; i < 5; i++){
          ...
          if(i%2 == 0) fs.writeFileSync('lastPost.json', JSON.stringify({ currentPost }));
        }
        page.close()
        bot.telegram.sendMessage('826003467', `${currentPost}`)
        currentPost+=5
      }
      else{
